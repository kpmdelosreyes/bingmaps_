...
Web.Browser.AttachMozillaCompatibility = function (w)
{
	...
	function Map(el,mozillaType, callback)
	{
		...
		// dirty hack to make scroll wheel work for Firefox but not break Safari
	...
	function CaptureMouse(ev)
	{
		if (m_Capturing)
		{
			...
			// sstchur:  the following works around a bug in Firefox3,
			// where incorrect .pageX/Y values are returned for manually dispatched events.			
			oEvent.__defineGetter__('pageX', function() { return this.clientX + window.pageXOffset;	});
			oEvent.__defineGetter__('pageY', function() { return this.clientY + window.pageYOffset; });
			//
	...
	// See VE specific behavior below
	...
	// Removing mouse button validation for Virtual Earth, as it alters the default behavior of 
	// right and middle click events in Firefox
